...
JNIEXPORT jint JNICALL Java_org_beeware_rubicon_Python_init(JNIEnv *env, jobject thisObj, jstring pythonHome, jstring pythonPath, jstring rubiconLib) {
    int ret = 0;
        ...
        if (pythonPath) {
            ...
            python_path = (*env)->GetStringUTFChars(env, pythonPath, NULL);
            wpython_path = Py_DecodeLocale(python_path, NULL);
            Py_SetPath(wpython_path);
        }

        LOG_I("Python Home: %s", python_home);
        LOG_I("Python Path: %s", python_path);
